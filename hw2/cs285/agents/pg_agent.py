...
class PGAgent(nn.Module):
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [self._discounted_return(rwd) for rwd in rewards]
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # DONE: use the helper function self._discounted_reward_to_go to calculate the Q-values
            q_values = [self._discounted_reward_to_go(rwd) for rwd in rewards]
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    val_next = 0 if terminals[i] else values[i+1]
                    delta = rewards[i] + self.gamma * val_next - values[i]
                    adv_next = 0 if terminals[i] else advantages[i+1]
                    advantages[i] = delta + self.gamma * self.gae_lambda * adv_next
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        total_return = np.full_like(rewards, np.sum(discounts * rewards))
        return total_return
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        t, tt = np.indices((T, T))
        dt = tt - t
        discounts = np.where(dt >= 0, self.gamma ** dt, 0)
        total_return = discounts @ rewards.transpose()
        return total_return
