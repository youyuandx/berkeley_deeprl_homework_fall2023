...
def run_training_loop(params):
    ...
    # DONE: Implement missing functions in this class.
    actor = MLPPolicySL(
        ac_dim,
        ob_dim,
        params['n_layers'],
    ...
    for itr in range(params['n_iter']):
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          sample_idxs = np.random.permutation(np.arange(len(replay_buffer.obs)))
          ob_batch, ac_batch = torch.from_numpy(replay_buffer.obs[sample_idxs]), torch.from_numpy(replay_buffer.acs[sample_idxs])
